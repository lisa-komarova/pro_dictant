    ...
    return Column(
      ...
      children: <Widget>[
        for (int weekNumber = 0;
            weekNumber < separatedDate.length;
            weekNumber++)
          HeatMapCalendarRow(
            startDate: separatedDate[weekNumber].keys.first,
            endDate: separatedDate[weekNumber].values.first,
            colorMode: colorMode,
            size: size,
            fontSize: fontSize,
            ...
            datasets: Map.from(datasets ?? {})
              ..removeWhere(
                (key, value) => !(key
                            .isAfter(separatedDate[weekNumber].keys.first) &&
                        key.isBefore(separatedDate[weekNumber].values.first) ||
                    key == separatedDate[weekNumber].keys.first ||
                    key == separatedDate[weekNumber].values.first),
              ),
            weekNumber: weekNumber, // <-- вот сюда передаём
          ),
      ],
    );
  }
